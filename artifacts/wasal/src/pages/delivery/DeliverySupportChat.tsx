...
const DeliverySupportChat = () => {
  ...
  const saveWaNumber = async () => {
    ...
    try {
      ...
      toast({
        title: "خطأ في حفظ رقم واتساب",
        description: "تأكد من وجود صلاحية إدارة الإعدادات",
        variant: "destructive",
      });
    }
    setWaSaving(false);
  ...
  useEffect(() => {
    const channel = supabase
      .channel("delivery-support-watch")
          ...
          <div className="flex flex-col flex-1 min-w-0" style={{ background: "var(--chat-bg, #f0f2f5)" }}>
            ...
            <div className="flex-1 overflow-y-auto px-4 py-4 space-y-2">
              {msgLoading ? (
                ...
                messages.map((msg) => {
                  const isStaff = msg.sender_id !== selectedConv.user_id;
                  return (
                    <div key={msg.id} className={`flex ${isStaff ? "justify-start" : "justify-end"}`}>
                      ...
                      >
                        ...
                        <p className={`text-[10px] mt-1 ${isStaff ? "text-muted-foreground text-right" : "opacity-70 text-left"}`}>
      ...
      <Dialog open={waEditOpen} onOpenChange={setWaEditOpen}>
        <DialogContent className="max-w-sm" dir="rtl">
          ...
          <div className="space-y-4 pt-1">
            ...
            >
              {waSaving ? <Loader2 className="w-4 h-4 animate-spin" /> : <WhatsAppIcon className="w-4 h-4" />}
              حفظ الرقم
            </Button>
          </div>
        </DialogContent>
...
export default DeliverySupportChat;
