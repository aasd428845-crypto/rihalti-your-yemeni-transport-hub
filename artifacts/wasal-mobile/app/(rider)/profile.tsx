    ...
    <ScrollView style={styles.container} contentContainerStyle={{ paddingBottom: 100 }}>
      ...
      <View style={styles.infoCard}>
        {[
          ...
          <View key={i} style={[styles.row, i < arr.length - 1 && { borderBottomWidth: 1, borderBottomColor: colors.light.border }]}>
            <Text style={styles.rowVal}>{r.value}</Text>
            <Text style={styles.rowLbl}>{r.label}</Text>
            <Feather name={r.icon as any} size={16} color={colors.light.mutedForeground} />
          </View>
        ))}
      </View>
...
const styles = StyleSheet.create({
  ...
  infoCard: { marginHorizontal: 16, marginTop: 16, backgroundColor: colors.light.card, borderRadius: 14, paddingHorizontal: 16 },
  ...
  logoutBtn: { margin: 16, flexDirection: "row", justifyContent: "center", alignItems: "center", gap: 8, backgroundColor: colors.light.card, borderRadius: 12, paddingVertical: 16, borderWidth: 1, borderColor: colors.light.destructive + "44" },
