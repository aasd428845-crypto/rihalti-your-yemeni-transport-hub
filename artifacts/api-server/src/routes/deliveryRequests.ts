...
// ─── Offer validity — mirrors isOfferCurrentlyActive in deliveryOffersApi.ts ──
...
//
// Accepts only non-price fields from the client. Computes price server-side
// using the real price_per_km from partner_settings and the actual coordinates.
// Mirrors the security model of api/orders/create (restaurant orders).
//
// Body (non-price only):
//   delivery_company_id, customer_name, customer_phone, customer_address,
//   delivery_lat, delivery_lng, pickup_address, pickup_lat, pickup_lng,
//   service_type, item_size, item_description, image_url, notes,
//   sender_name, sender_phone, recipient_name, recipient_phone,
//   item_weight, item_dimensions, payment_method,
//   applied_offer_id (optional — verified server-side)
//
// Returns: { order, awaitingPricing, computedFee, distanceKm }
router.post("/delivery-requests/create", async (req, res) => {
  ...
  // auth.getUser(jwt) validates the token against Supabase Auth
  ...
  // 4. Determine awaitingPricing — true when coordinates are missing or company has no pricing
  ...
  if (!awaitingPricing) {
    ...
    if (applied_offer_id) {
      ...
      const isShipmentOffer =
        offerRecord?.scope === "shipment" || !offerRecord?.scope;
